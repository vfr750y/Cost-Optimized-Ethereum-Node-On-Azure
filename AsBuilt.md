...
|   1.3    | Create the Storage Account for state management  <br>``` STORAGE_NAME="stethterraformstate" ``` <br>```az storage account create ``` <br> ```--name $STORAGE_NAME --resource-group rg-lodestar-node``` --location eastus --sku Standard_LRS
